...
# ID mapping for stable display IDs
_id_gen = count(1)
_id_map = {}
...
def process_video(input_path, output_path, enable_trajectories=True, progress_callback=None):
    
    # Initialize the YOLO model with GPU
    model = YOLO(r"E:\Foodball_Analysis_System\models\fine_tuning_yolo11x\best.pt")
    model.to('cuda')  # Move model to GPU
    
    # Reset the ID generator and map for each video
    global _id_gen, _id_map
    _id_gen = count(1)
    _id_map = {}
    ...
    # Initialize team assigner and other variables
    team_assigner = TeamAssigner()
    team_colors_assigned = False
    trajectories = defaultdict(list) if enable_trajectories else None
    frame_count = 0
    ...
    for res in results:
        ...
        names[1] = "player"      # rename goalkeeper to player for display
        ...
        for (x1, y1, x2, y2), tid, c, conf in zip(boxes, ids, clses, confs):
            ...
            if enable_trajectories and int(c) in (PLAYER_CLASS_ID, GOALKEEPER_CLASS_ID):
